        ...
        with lock:
            ...
            if state == "PRESENT":
                ...
                for tag, t_time in pending_tags:
                    print(f"[MATCH] Comparing tag time {timestamp(t_time)} to slot time {timestamp(now)}")
                    if abs(now - t_time) <= MATCH_WINDOW_SECONDS:
                        matched_tag = tag 
                        print(f"[MATCH] Tag Pulled: {matched_tag}")
                        break
                if matched_tag:
                    ...
                    print("[FIREBASE] Added to CurrentChargingList")
                    ...
                    if getCurrentChargingRecords is None: #Incase this is the first charge record for this battery/tag
                      ...
                      print(f"[FIREBASE] First record for {matched_tag} created")
                    ...
                    print(f"[FIREBASE] Checking for name for {matched_tag}")
                    if not name_ref.get():
                        # Trigger the frontend to prompt naming
                        print(f"[FIREBASE] No name found for {matched_tag}, prompting for name.")
                ...
                if prev_tag:
                    ...
                    print("[FIREBASE] Removed from CurrentChargingList")
                    ...
                    print(f"[FIREBASE] Duration for {prev_tag} was {duration}")
                    ...
                    print(f"[FIREBASE] Updated record for {prev_tag} with end time and duration")
                    ...
                    print(f"[FIREBASE] Pulled Minimum Time Setting {minTimeSetting} seconds")
...
def listen_rfid():
    while True:
        ...
        print(f"[RFID] Input Received, added to buffer: {tag_buffer}")
...
def led_manager_loop():
    ...
    print("[HEARTBEAT] Heartbeat reset")
    ...
    while True:
        ...
        for tag, data in batteries.items():
            ...
            if data.get("IsCharging") and data.get("ChargingSlot") is not None: #if its currently charging
                ...
                print(f"[LED] Battery {tag} in slot {data['ChargingSlot']} is charging")
        ...
        print(f"[LED] Next slot to pick: {pick_next_slot}")
